...
//=========================================================================================================
		...
		// real coordinates
		for(int k=i; k<i+3; k++)
			wc_out <<mdframe.name[k]<<"(real)" << " "<< mdframe.x[k] <<  " " << mdframe.y[k] <<  " " << mdframe.z[k] << std::endl;
		...
		for(int k=i; k<i+3; k++)
		{
			...
			r_lab -= ref; 
			wc_out <<mdframe.name[k]<<"(lab) " << " "<< r_lab[0] <<  " " << r_lab[1] <<  " " << r_lab[2] << std::endl;
		}
		...
		for(int k=i; k<i+3; k++)
		{
			...
			wc_out <<mdframe.name[k]<<"(mol) " << " "<< r_mol[0] <<  " " << r_mol[1] <<  " " << r_mol[2] << std::endl;
		}
		...
		if(nbrlist_wc[i].size()>0)
		{
			...
			{
				int iw = std::get<1>(l);
				Eigen::Vector3d r_lab(mdframe.x[iw], mdframe.y[iw], mdframe.z[iw]);
				r_lab -= ref; 
				for (int ia=0; ia<3; ia++) r_lab(ia) = mdframe.apply_pbc(r_lab(ia), mdframe.lattice[ia]);
				auto const r_mol = Minv*r_lab;
				wc_out << "WC" << std::to_string(counter++) << " " << r_mol[0] <<  " " << r_mol[1] <<  " " << r_mol[2] << std::endl;
			}
		} else {
